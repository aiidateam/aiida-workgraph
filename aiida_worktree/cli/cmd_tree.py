...
def valid_projections():
    ...
    """
    return WorkTreeQueryBuilder.valid_projections
...
def default_projections():
    ...
    This indirection is necessary to prevent loading the imported module which slows down tab-completion.
    """
    return WorkTreeQueryBuilder.default_projections
...
def process_list(
    ...
    """Show a list of running or terminated WorkTree processes.
...
def process_show(processes):
    ...
    for process in processes:
        wt = WorkTree.load(process.pk)
        wt.show()


@worktree_tree.command("report")
@arguments.PROCESSES()
@click.option(
    "-i",
    "--indent-size",
