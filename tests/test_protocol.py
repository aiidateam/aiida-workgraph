...
def test_pw_relax_protocol(structure_si):
    ...
    assert "base_final_scf" in wg.tasks["pw_relax1"].node.inputs
    energy = wg.tasks["pw_relax1"].node.outputs.output_parameters["energy"]
...
def test_pw_relax_protocol_pop(structure_si):
    ...
    wg.submit(wait=True, timeout=200)
    assert "base_final_scf" not in wg.tasks["pw_relax1"].node.inputs
    energy = wg.tasks["pw_relax1"].node.outputs.output_parameters["energy"]
    assert np.isclose(energy, -308.46, atol=0.1)
