...
def test_while(decorated_add, decorated_multiply, decorated_compare):
    ...
    wg.context = {"n": 1}
    wg.max_iteration = 10
    wg.tasks.new(decorated_compare, name="compare1", x="{{n}}", y=50)
...
def test_while_graph_builder(decorated_add, decorated_multiply, decorated_compare):
    ...
    def my_while(n=0, limit=100):
        ...
        wg.context = {"n": n}
        wg.max_iteration = 10
        wg.tasks.new(decorated_compare, name="compare1", x="{{n}}", y=orm.Int(limit))
        multiply1 = wg.tasks.new(
            decorated_multiply, name="multiply1", x="{{ n }}", y=orm.Int(2)
        )
        add1 = wg.tasks.new(decorated_add, name="add1", y=3)
    ...
    # -----------------------------------------
    wg = WorkGraph("while")
    add1 = wg.tasks.new(decorated_add, name="add1", x=orm.Int(25), y=orm.Int(25))
    my_while1 = wg.tasks.new(my_while, n=orm.Int(1))
    add2 = wg.tasks.new(decorated_add, name="add2", y=orm.Int(2))
