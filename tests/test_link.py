...
def test_multiply_link() -> None:
    ...
    float1 = wg.add_task("AiiDANode", pk=Float(1.0).store().pk)
    float2 = wg.add_task("AiiDANode", pk=Float(2.0).store().pk)
    float3 = wg.add_task("AiiDANode", pk=Float(3.0).store().pk)
    gather1 = wg.add_task("AiiDAGather", "gather1")
