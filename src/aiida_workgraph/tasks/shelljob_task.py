...
def _build_shelljob_nodespec(
    *,
    ...
    in_spec = merge_spec(in_spec, additions_in)

    # Ensure stdout/stderr outputs
    additions_out = namespace(stdout=Any, stderr=Any)
    out_spec = merge_spec(out_spec, additions_out)
    ...
    if outputs:
        ...
        out_spec = merge_spec(out_spec, outputs)

    if parser_outputs:
        out_spec = merge_spec(out_spec, parser_outputs)
