...
class AwaitableManager:
    ...
    def on_awaitable_finished(self, awaitable: Awaitable) -> None:
        ...
        if isinstance(awaitable.pk, int):
            self.logger.info(
                f'received callback that awaitable with key {awaitable.key} and pk {awaitable.pk} has terminated'
            )
            try:
            ...
            self.logger.info(f'received callback that awaitable {awaitable.key} has terminated')
            try:
