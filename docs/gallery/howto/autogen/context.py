...
# 2. Setting the ``ctx`` attribute of the WorkGraph directly, on initialization
...
# 3. Use the `update_ctx` task to set either the task result or a constant value to the ctx.
...
# Nested context keys
# ~~~~~~~~~~~~~~~~~~~
# To organize the context data (e.g. group data), The keys may contain dots `.`, which will introduce a hierarchical structure
# Here is an example, to group the results of all add tasks to `ctx.sum`:
#
