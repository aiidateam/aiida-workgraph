...
def add_multiply_group(x, y, z, t=2):
    ...
    wg.links.new(add1.outputs["result"], multiply.inputs["y"])
    return wg
...
arithmetic_add0 = wg.nodes.new(
    ArithmeticAddCalculation, "arithmetic_add_0", x=x, y=x, code=code
)
arithmetic_add1 = wg.nodes.new(
    ArithmeticAddCalculation, "arithmetic_add_1", x=x, y=x, code=code
)
